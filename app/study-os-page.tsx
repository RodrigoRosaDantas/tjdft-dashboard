...
export default function StudyOsPage({view,root=false}:{view:View;root?:boolean}) {
  ...
  return <div className="study-os-shell">
    ...
    <section className="os-metrics">
      ...
      <Metric label="Erros ativos" value={count(m.errorCount)} detail={m.errorCount==null?(m.errorRecordsPresent===false?"sem registros; contagem não calculável":"aguardando snapshot ou dados classificados") : m.errorCount>0 ? "registros ativos no Caderno" : "nenhum ativo entre os registros consultados"}/>
    </section>
    ...
    {view==="home" && <div className="os-grid">
      ...
      <section className="os-card"><h2>Onde estou errando?</h2><p>{m.errorCount!=null?`${m.errorCount} erro(s) ativo(s).`:m.errorRecordsPresent===false?"Sem registros de erro; contagem ativa não calculável.":"Caderno operacional sem contagem confiável."} {m.weaknesses.length?`${m.weaknesses.length} fragilidade(s) com amostra suficiente.`:"Sem fragilidade estatística sustentada."}</p><a href={route(root,"erros")}>Caderno de erros →</a></section>
    ...
    {view==="hoje" && <div className="os-grid">
      ...
      <section className="os-card"><h2>Erro prioritário</h2><p>{m.activeErrors[0]?<><b>{m.activeErrors[0].topic||m.activeErrors[0].subject}</b> · {m.activeErrors[0].severity} {m.activeErrors[0].action?"· "+m.activeErrors[0].action:""}</>:m.errorCount===0?"Nenhum erro ativo entre os registros consultados.":m.errorRecordsPresent===false?"Sem registros no Caderno; não há base para avaliar erros ativos.":"Nenhum erro ativo publicável no resumo operacional."}</p><a href={route(root,"erros")}>Caderno de erros →</a></section>
    ...
    {view==="mentor" && <div className="os-grid">
      ...
      <section className="os-card os-wide"><h2>Evidências usadas</h2><p>{m.execution.questions!=null?String(m.execution.questions)+" questões · "+count(m.execution.sessions)+" sessão(ões) registrada(s) · "+percent(m.execution.precision)+" de precisão.":"Sem amostra de questões suficiente."} {m.errorCount!=null?String(m.errorCount)+" erro(s) ativo(s).":m.errorRecordsPresent===false?"Sem registros no Caderno; contagem ativa não calculável.":"Caderno sem contagem confiável."}</p>{m.nextAction.evidence?.length?<ul>{m.nextAction.evidence.map((item:string,i:number)=><li key={i}>{item}</li>)}</ul>:null}</section>
    </div>}
